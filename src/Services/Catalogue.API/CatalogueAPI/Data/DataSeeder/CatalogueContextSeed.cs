...
namespace CatalogueAPI.Data.DataSeeder
{
    public class CatalogueContextSeed
    {
        ...
        {
            bool doesProductExist = productCollection.Find(x => true).Any();
            if (!doesProductExist)
            {
                productCollection.InsertMany(seedNewProducts());
            }
        }

        private static IEnumerable<Product> seedNewProducts()
        {
